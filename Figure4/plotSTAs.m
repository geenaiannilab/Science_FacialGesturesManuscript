...
cellPairs = data.cellPairs;
bhvs2plot = data.bhvs2plot;

corrLSSTAs = zeros(1, length(cellPairs));
corrThrSTAs = zeros(1, length(cellPairs));

counter = 1;
for  pp = 1:length(cellPairs)

    corrLSSTAs(pp) = corr2(data.dataLSOut(:,:,counter),data.dataLSOut(:,:,counter+1));
    corrThrSTAs(pp) = corr2(data.dataThrOut(:,:,counter), data.dataThrOut(:,:,counter+1)); 
    counter = counter +2;
end
